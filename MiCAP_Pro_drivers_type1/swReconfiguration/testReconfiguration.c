...
u32 presort(const lutlocation *location_array,
		...
		u32 group_sizes[max_TLUTs],
		u32 number_of_tluts) {

	u8 is_sorted[max_TLUTs] = {};
...
void reconfigure_group(XGpio* gpio_InstancePtr, XAxiDma* dma_InstancePtr,LUT_config_type lut_configs[NUMBER_OF_INSTANCES*NUMBER_OF_TLUTS_PER_INSTANCE],
	...
	for(i=0;group_sizes[i]!=0;i++) {
		XStatus Status;
		Status = MiCAP_Custom_SetMultiClbBits(gpio_InstancePtr,dma_InstancePtr,lut_config_p, group_sizes[i]);
...
void test_fir(XGpio* gpio_InstancePtr, XAxiDma* dma_InstancePtr) {
	...
	presort((lutlocation*)location_array, (u8*)output, lut_configs, group_sizes, max_TLUTs);
	xil_printf("After presort.. \n\r\n\r");

	Start_Timer();
    start1 = Get_Timer();
	reconfigure_group(gpio_InstancePtr,dma_InstancePtr, lut_configs, group_sizes);
    stop1= Stop_Timer();
    exec1 = stop1 - start1;
    xil_printf("Reconfigure max_TLUTs TLUTs = %d\n\r\n\r", exec1);
	//Remove the reset
	...
	xil_printf("Starting FIR test round 2. . . \n\r\n\r");
		...
		//	presort(HwIcap_p, (lutlocation*)location_array, (u8*)output, lut_configs, group_sizes, max_TLUTs);
			xil_printf("After presort.. \n\r\n\r");
